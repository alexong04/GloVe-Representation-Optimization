...
class ExperimentRun:
	...
	def __init__(self, config_filepath):
		...
		self.pretrained_word_vectors = load_word_vectors(pretrained_vectors_filepath, None)
...
def load_word_vectors(file_destination, vocabulary=None):
    print("Loading pretrained word vectors from", file_destination)
    word_dictionary = {}
    try:
        with open(file_destination, "r", encoding="utf-8") as f:
            for line in f:
                ...
                key = line[0].lower()
                if vocabulary is None or key in vocabulary:
                    word_dictionary[key] = numpy.fromstring(line[1], dtype="float32", sep=" ")
    except Exception as e:
        print("Word vectors could not be loaded from:", file_destination, e)
        return {}
    print(len(word_dictionary), "vectors loaded from", file_destination)            
    return normalise_word_vectors(word_dictionary)
...
def print_word_vectors(word_vectors, write_path):
	...
	with open(write_path, "wb") as f_write:
		for key in word_vectors:
			...
			line = key + " " + " ".join(map(str, numpy.round(word_vectors[key], decimals=6))) + "\n" 
			f_write.write(line.encode("utf-8"))
...
def compute_vsp_pairs(word_vectors, vocabulary, rho=0.2):
	...
	for left_range in range(range_count):
		for right_range in range(left_range, range_count):
			...
			full_left_range = list(range(list_of_ranges[left_range][0], list_of_ranges[left_range][1]))		
			full_right_range = list(range(list_of_ranges[right_range][0], list_of_ranges[right_range][1]))
...
# 	"""
# 	fread_simlex = open("counter-fitting/linguistic_constraints/SimLex-999.txt", "rb")
# 	pair_list = []
...
# 				pair_list.append( ((word_i.lower(), word_j.lower()), score) )
# 		line_number += 1

# 	pair_list.sort(key=lambda x: - x[1])

# 	f_out_simlex = open("counter-fitting/results/simlex_ranking.txt", "wb")
# 	f_out_counterfitting = open("counter-fitting/results/counter_ranking.txt", "wb")
...
# 		print(idx, ":", clean_elem, file=f_out_simlex)

# 	for idx, element in enumerate(extracted_list):
# 		clean_elem = str(parse_pair(element[0])) + " : " + str(round(element[1], 2))
# 		print(idx, ":", clean_elem, file=f_out_counterfitting)
...
def run_experiment(config_filepath):
	...
	print_word_vectors(transformed_word_vectors, "counter-fitting/results/c_glove30.txt")
...
def main():
	...
	try:
		...
		config_filepath = "counter-fitting/experiment_parameters.cfg"
